...
if ! (kubectl get secret s3proxy-tls -o yaml | yq e '.data."ca.crt"' - | base64 -d >s3proxy-ca.crt); then
...
while true; do
	...
	if ! echo | openssl s_client -connect "$port_forward_ip":443 -brief -verify_return_error -CAfile s3proxy-ca.crt; then
